...
# 확률 변수의 사용
- 확률 질량 함수에서 확률 변수는 이산형이고, 확률 밀도 함수에서 확률변수는 연속형임
- 가장 큰 차이점은 연속성임
- 연속변수 일 때 적분을 통해 면적을 구하면 구간별 확률값이 됨

# 가설 검정
- 어떤 사실을 주장하려면 이미 일반화된 ‘영가설’을 새로운 사실을 주장하는 ‘대립가설’을 세워야함
- 표집을 통해 표본을 얻고, 새로운 데이터를 얻음
- 표본에서 얻은 데이터로 영가설을 증명 ( 참 일수 없다는 것이 확률적으로 나타남)
- 영가설은 기각되고 자신이 주장한 대립가설이 일반적 사실로 받아 들여짐
- 영가설과 대립가설을 서로 비교 , 그 비교를 확률로서 한다는 것이 가장 중요함

# 난수(랜덤값) 생성하기
- R은 정규분포를 따르는 난수, 이항분포를 따르는 난수
- 정규분포 : rnorm()
- 이항분포 : rbinorm()
- 균등 ‘난수(랜덤값)’ : runif()
...
pnorm(370, meanb=350, sd=75) - pnorm(200, mean=350, sd=75)
# 한줄로 나타내고 싶다면?
